...
console.log("=== LOAD FEEDBACKS ===");
    console.log("Loaded feedbacks count:", a.length);
    ...
    console.log("Previous feedbacks count:", feedbacks.length);
      setFeedbacks((prevFeedbacks) => [feedback, ...prevFeedbacks]);
      console.log("Updated feedbacks count:", feedbacks.length + 1);
    });
  ...
  const handleFormSubmit = (e) => {
    e.preventDefault();
    if(hall.length<1 || aHall.length<1 || problemStatement.length<1 || para.length<1 || link.length<1){
      console.error("Form submission error: Please fill all fields correctly.");
      alert("Please enter all required fields");
      return;
    }
  if(hall=== aHall){
      console.error("Form submission error: Pool cannot be the same as Against Pool.");
...
console.log("=== FORM SUBMISSION DEBUG ===");
  ...
  console.log("Current feedbacks before submit:", feedbacks.length);
...
console.log("Switched to tab: my-contentions");
  console.log("Current feedbacks after submit:", feedbacks.length);

};


  return (
    <div className="portal-box" style={{ backgroundColor: "#000000", color: "white" , marginTop:"0px"}}>
      <div className="navbar" style={{ display: "flex",  color:"white", flexDirection: "row",  border:"2px solid #7f7fff", borderTop:"none", borderLeft:"none", borderRight:"none",  alignItems: "center", backgroundColor:"#000000", marginTop:"0px", position:"fixed",  }}>
        ...
        {getUserDetails().role == "user" && (
          ...
          <div className="tab-navigation" style={{ 
            ...
            padding:"0"}}>
              ...
              style={{
                padding: "15px 30px",
              backgroundColor: activeTab === "submit" ? "#7f7fff" : "transparent",
...
<button
  className={`tab-button ${activeTab === "my-contentions" ? "active":""}`}
  onClick={() => setActiveTab("my-contentions")} 
  style={{
    padding: "15px 30px",
    backgroundColor: activeTab === "my-contentions" ? "#7f7fff" : "transparent",
...
<button
  className={`tab-button ${activeTab === "against-me" ? "active":""}`}
  onClick={() => setActiveTab("against-me")} 
  style={{
    padding: "15px 30px",
    backgroundColor: activeTab === "against-me" ? "#7f7fff" : "transparent",
  ...
  {getUserDetails().role==="user" && activeTab ==="submit" && (
    <div className="contention-form">
      <form className="feedback-input" onSubmit={handleFormSubmit}>
              <label htmlFor="pool-select" style={{color:"white", backgroundColor:"white" ,width:"100%"}}> 
                <select
...
{getUserDetails().role === "user" && activeTab === "my-contentions" && (
          <MyContentions feedbacks={feedbacks} />
        )}


        {getUserDetails().role === "user" && activeTab === "against-me" && (
          <ContentionsAgainstMe feedbacks={feedbacks} socket={socket} />
        )}
...
{getUserDetails().role === "admin" && (
          <div className="container">
            <h2 style={{ color: "white", marginBottom: "20px" }}>All Contentions</h2>
            {feedbacks.length > 0 ? (
                ...
                return (
                  <div className="feedback-card" key={feedback._id}>
                    <div className="feedback-header">
                      ...
                      <p><strong>From:</strong> {feedback.hall}</p>
                      <p><strong>Against:</strong> {feedback.againstHall}</p>
                      <p><strong>Problem:</strong> {feedback.problemStatement}</p>
                      {feedback.para && <p><strong>Description:</strong> {feedback.para}</p>}
                      {feedback.link && (
                        <p>
                          <strong>Link:</strong>{" "}
                      ...
                      <span className="username">Submitted by: {feedback.username}</span>
                      ...
                      <button
                        ...
                        onClick={() => {
                          socket.emit("change_status", {
                            id: feedback._id,
                            status: feedback.status === "pending" ? "reviewed" : "pending",
                          });
                        }}
                      >
                        Toggle Review
